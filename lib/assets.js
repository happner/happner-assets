...
Assets.load = function(happn) {
  ...
  for (var i = 0; i < masks.length; i++) {
    if (masks[i].match(/\/\/localhost\//)) {
      var local = happn.info.datalayer.address.address + ':' + happn.info.datalayer.address.port;
      masks[i] = masks[i].replace(/\/\/localhost\//, '//' + local + '/');
    } else if (masks[i].match(/\/\/localhost$/)) {
      var local = happn.info.datalayer.address.address + ':' + happn.info.datalayer.address.port;
      masks[i] = masks[i].replace(/\/\/localhost/, '//' + local);
    }
  }
...
Assets.loadWeb = function(happn, type, i, url) {
  return promise(function(resolve, reject) {
    ...
    var result = function(err, response, body) {
      ...
      happn.assetRaw[type][i] = ['web', i, url, md5(body), body, isMini];
      resolve();
    }
      ...
      if (!url.match(match)) {
        ...
        if (type == 'js') {
          try {
            ...
            happn.log.warn('failed to minify %s', url);
            content += '\n' + data;
          }
          return;
        }
